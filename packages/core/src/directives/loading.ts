...
function ensureMask(el: HTMLElement){
  let mask = el.querySelector(':scope > .k-loading-mask') as HTMLElement | null
  if (!mask) {
    mask = document.createElement('div'); mask.className = 'k-loading-mask'
    const ring = document.createElement('span'); ring.className = 'k-spinner-ring'
...
function removeMask(el: HTMLElement){ const m = el.querySelector(':scope > .k-loading-mask'); if (m) m.remove() }

export const vLoading: Directive<HTMLElement, boolean> = {
  mounted(el, b){ el.classList.add('k-loading-host'); if (b.value) ensureMask(el) },
