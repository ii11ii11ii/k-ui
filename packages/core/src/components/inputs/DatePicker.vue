...
<template>
  <div class="k-datepicker">
    <button type="button" :class="['k-dp-field', invalid ? 'is-invalid' : '', disabled ? 'is-disabled' : '']" :disabled="disabled" @click="open = !open">
      ...
      <span :class="['k-dp-text', !modelValue ? 'is-ph' : '']">{{ display || placeholder }}</span>
      <span v-if="clearable && modelValue" class="k-dp-clear" @click="clear"><Icon name="close" :size="13" /></span>
    </button>
    <template v-if="open">
