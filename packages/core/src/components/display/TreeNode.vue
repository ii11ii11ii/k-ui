...
<template>
  <div class="k-tree-node">
    <div :class="['k-tree-row', selectedKey === node.key ? 'is-selected' : '']" :style="{ paddingLeft: (level * 16 + 8) + 'px' }" @click="$emit('select', node)">
      <button v-if="hasChildren()" type="button" :class="['k-tree-caret', open ? 'is-open' : '']" @click.stop="toggle"><Icon name="chevronRight" :size="13" /></button>
      <span v-else class="k-tree-caret-spacer"></span>
      <Icon v-if="node.icon" :name="node.icon" :size="15" class="k-tree-ico" />
      <span class="k-tree-label">{{ node.label }}</span>
    </div>
    <div v-if="hasChildren() && open" class="k-tree-children">
